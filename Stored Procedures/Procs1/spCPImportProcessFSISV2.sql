...
CREATE    Proc [dbo].[spCPImportProcessFSISV2](	
	@pkCPImportBatch int
)
as
Declare
...
--from ProgramType
--where ProgramType = 'FSIS'
--
--select @pkProgramType = isnull(@pkProgramType, -1)
select @pkProgramType = 1
	...
	if @CaseNumPrevious <> @CaseNum begin
	 ...
	 -- did this person already process today?
		...
		Exec spCPImportCPClientCaseV2	
			...
			,@pkCPRefClientCaseProgramType = @pkProgramType --FSIS
			,@fkCPClientCaseHead = @pkCPClient
			,@pkrefClientCaseImportSource = 1
