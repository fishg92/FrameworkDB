...
CREATE    Proc [dbo].[spCPImportProcessMinorConsentV2]
(	
	@pkCPImportBatch int
)
...
Declare @HostName varchar(100)
		...
		,@pkCPImportMinorConsent decimal (18,0)
...
from ProgramType
where ProgramType = 'MinorConsent'
or ProgramType = 'Minor Consent'
...
Declare cImportKeys INSENSITIVE Cursor For
	Select pkCPImportMinorConsent
	From CPImportMinorConsent with (nolock)
...
Fetch Next From cImportKeys into @pkCPImportMinorConsent
Set @lf = @@Fetch_Status
...
begin
	Select 	
		...
		,@WorkerNum = WorkerNum	
	From	CPImportMinorConsent with (nolock)
	Where	pkCPImportMinorConsent = @pkCPImportMinorConsent
	...
	if @CaseNumPrevious <> @CaseNum begin
	 ...
	 -- did this person already process today? Will need to add other program type checks here
		if not exists (Select * From CPImportMinorConsent with (nolock)
...
--	from dbo.CPImportMinorConsent with (nolock)
--	where fkCPImportBatch = @pkCPImportBatch
...
--					select ic.CaseID 
--					from CPImportMinorConsent ic with (nolock)
...
--		inner join dbo.CPImportMinorConsent i with (nolock) on i.CaseID = j.StateCaseNumber
