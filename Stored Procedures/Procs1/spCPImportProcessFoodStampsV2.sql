...
CREATE    Proc [dbo].[spCPImportProcessFoodStampsV2]
(	
	@pkCPImportBatch int
)
...
Declare @HostName varchar(100)
		...
		,@pkCPImportFoodStamps decimal (18,0)
...
from ProgramType
where ProgramType = 'FoodStamps'
or ProgramType = 'Food Stamps'
...
Declare cImportKeys INSENSITIVE Cursor For
	Select pkCPImportFoodStamps
	From CPImportFoodStamps with (nolock)
...
Fetch Next From cImportKeys into @pkCPImportFoodStamps
Set @lf = @@Fetch_Status
...
begin
	Select 	
		...
		,@WorkerNum = WorkerNum	
	From	CPImportFoodStamps with (nolock)
	Where	pkCPImportFoodStamps = @pkCPImportFoodStamps
	...
	if @CaseNumPrevious <> @CaseNum begin
	 ...
	 -- did this person already process today? Will need to add other program type checks here
		if not exists (Select * From CPImportFoodStamps with (nolock)
...
--	from dbo.CPImportFoodStamps with (nolock)
--	where fkCPImportBatch = @pkCPImportBatch
...
--					select ic.CaseID 
--					from CPImportFoodStamps ic with (nolock)
...
--		inner join dbo.CPImportFoodStamps i with (nolock) on i.CaseID = j.StateCaseNumber
