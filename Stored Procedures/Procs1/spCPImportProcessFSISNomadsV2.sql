...
--exec dbo.spCPImportNomadsPending
CREATE    Proc [dbo].[spCPImportProcessFSISNomadsV2](	
	@pkCPImportBatch int
)
as
Declare
	...
	,@PreviousImportBatch int
	
select @PreviousImportBatch = MAX(pkCpImportBatch)
from	CPImportBatch
...
Declare cImportKeys INSENSITIVE Cursor For
	Select pkCPImportFSIS 
	From CPImportFSIS iCurrent with (nolock)
	Where fkCPImportBatch = @pkCPImportBatch 
	and not exists (select * from CPImportFSIS iLast
					where fkCPImportBatch = @PreviousImportBatch
					and DataChecksum = iCurrent.DataChecksum)
...
Fetch Next From cImportKeys into @pkCPImportFSIS
Set @lf = @@Fetch_Status
...
begin
	Select 	
		...
		,@WorkerNum = WorkerNum	
	From	CPImportFSIS with (nolock)
	Where	pkCPImportFSIS = @pkCPImportFSIS
	...
	if @CaseNumPrevious <> @CaseNum begin
	 ...
	 -- did this person already process today?
		...
		Exec spCPImportCPClientCaseV2	
			...
			,@pkCPRefClientCaseProgramType = 1 --FSISNomads
			,@fkCPClientCaseHead = @pkCPClient
			,@pkCPClientCase = @pkCPClientCase Output
			,@pkCPRefImportLogEventType = @ImportLogEventType Output	-- Move Logging to SP
			,@pkrefClientCaseImportSource = 3
	...
	Update CPImportFSIS Set ProcessDate = GetDate() Where pkCPImportFSIS = @pkCPImportFSIS
...
--	from dbo.CPImportFSIS with (nolock)
--	where fkCPImportBatch = @pkCPImportBatch
...
--					select ic.CaseID 
--					from CPImportFSIS ic with (nolock)
...
--		inner join dbo.CPImportFSIS i with (nolock) on i.CaseID = j.StateCaseNumber
