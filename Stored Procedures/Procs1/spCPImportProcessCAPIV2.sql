...
CREATE    Proc [dbo].[spCPImportProcessCAPIV2]
(	
	@pkCPImportBatch int
)
...
Declare cImportKeys INSENSITIVE Cursor For
	Select pkCPImportCAPI
	From CPImportCAPI with (nolock)
...
Fetch Next From cImportKeys into @pkCPImportCAPI
Set @lf = @@Fetch_Status
...
begin
	Select 	
		...
		,@WorkerNum = WorkerNum	
	From	CPImportCAPI with (nolock)
	Where	pkCPImportCAPI = @pkCPImportCAPI
...
--	from dbo.CPImportCAPI with (nolock)
--	where fkCPImportBatch = @pkCPImportBatch
...
--					select ic.CaseID 
--					from CPImportCAPI ic with (nolock)
...
--		inner join dbo.CPImportCAPI i with (nolock) on i.CaseID = j.StateCaseNumber
