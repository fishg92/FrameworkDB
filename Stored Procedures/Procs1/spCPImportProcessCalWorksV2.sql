...
CREATE    Proc [dbo].[spCPImportProcessCalWorksV2]
(	
	@pkCPImportBatch int
)
...
Declare @HostName varchar(100)
		...
		,@pkCPImportCalWorks decimal (18,0)
		,@ImportLogEventType decimal(18,0)
		,@EffectiveDateOfDelete datetime
...
Declare cImportKeys INSENSITIVE Cursor For
	Select pkCPImportCalWorks
	From CPImportCalWorks with (nolock)
...
Fetch Next From cImportKeys into @pkCPImportCalWorks
Set @lf = @@Fetch_Status
...
begin
	Select 	
		...
		,@WorkerNum = WorkerNum	
	From	CPImportCalWorks with (nolock)
	Where	pkCPImportCalWorks = @pkCPImportCalWorks
	...
	if @CaseNumPrevious <> @CaseNum begin
	 ...
	 -- did this person already process today? Will need to add other program type checks here
		if not exists (Select * From CPImportCalWorks with (nolock)
...
--	from dbo.CPImportCalWorks with (nolock)
--	where fkCPImportBatch = @pkCPImportBatch
...
--					select ic.CaseID 
--					from CPImportCalWorks ic with (nolock)
...
--		inner join dbo.CPImportCalWorks i with (nolock) on i.CaseID = j.StateCaseNumber
