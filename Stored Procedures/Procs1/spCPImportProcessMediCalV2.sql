...
CREATE    Proc [dbo].[spCPImportProcessMediCalV2]
(	
	@pkCPImportBatch int
)
...
Declare @HostName varchar(100)
		...
		,@pkCPImportMediCal decimal (18,0)
...
Declare cImportKeys INSENSITIVE Cursor For
	Select pkCPImportMediCal
	From CPImportMediCal with (nolock)
...
Fetch Next From cImportKeys into @pkCPImportMediCal
Set @lf = @@Fetch_Status
...
begin
	Select 	
		...
		,@WorkerNum = WorkerNum	
	From	CPImportMediCal with (nolock)
	Where	pkCPImportMediCal = @pkCPImportMediCal
	...
	if @CaseNumPrevious <> @CaseNum begin
	 ...
	 -- did this person already process today? Will need to add other program type checks here
		if not exists (Select * From CPImportMediCal with (nolock)
...
--	from dbo.CPImportMediCal with (nolock)
--	where fkCPImportBatch = @pkCPImportBatch
...
--					select ic.CaseID 
--					from CPImportMediCal ic with (nolock)
...
--		inner join dbo.CPImportMediCal i with (nolock) on i.CaseID = j.StateCaseNumber
