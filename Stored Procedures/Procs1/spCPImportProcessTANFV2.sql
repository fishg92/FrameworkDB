...
CREATE    Proc [dbo].[spCPImportProcessTANFV2]
(	
	@pkCPImportBatch int
)
...
Declare cImportKeys INSENSITIVE Cursor For
	Select pkCPImportTANF
	From CPImportTANF with (nolock)
...
Fetch Next From cImportKeys into @pkCPImportTANF
Set @lf = @@Fetch_Status
...
begin
	Select 	
		...
		,@WorkerNum = WorkerNum	
	From	CPImportTANF with (nolock)
	Where	pkCPImportTANF = @pkCPImportTANF
	...
	if @CaseNumPrevious <> @CaseNum begin
	 ...
	 -- did this person already process today? Will need to add other program type checks here
		if not exists (Select * From CPImportTANF with (nolock)
...
--	from dbo.CPImportTANF with (nolock)
--	where fkCPImportBatch = @pkCPImportBatch
...
--					select ic.CaseID 
--					from CPImportTANF ic with (nolock)
...
--		inner join dbo.CPImportTANF i with (nolock) on i.CaseID = j.StateCaseNumber
